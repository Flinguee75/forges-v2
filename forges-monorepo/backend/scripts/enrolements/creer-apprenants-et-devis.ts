...
if (fileFlag === -1 || !args[fileFlag + 1]) {
  console.error('Usage: creer-apprenants-et-devis.ts --file <chemin/vers/fichier.csv> --context <chemin/vers/contexte.json> [--dry-run]');
  process.exit(1);
}
...
async function main() {
  ...
  console.log('\n' + '═'.repeat(72));
  console.log('  FORGES — Script apprenants + facture PDF');
...
if (require.main === module) {
  main().catch((err) => {
    console.error('Erreur script apprenants + facture:', err.message);
    process.exit(1);
  });
}
