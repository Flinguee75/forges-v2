...
export default function UtilisateursList() {
  ...
  useEffect(() => {
    execute(() => utilisateursApi.getAll(), {
      onSuccess: (data) => {
        setUsers(data?.users || []);
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <div className="rounded-lg bg-white p-6 shadow">
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <h2 className="mt-3 text-2xl font-semibold text-primary">Utilisateurs backoffice</h2>
            <p className="mt-2 text-subtext">Gérez les comptes SUPERVISEUR, AGENT, RESPONSABLE et GESTIONNAIRE.</p>
          </div>
          <Button data-testid="btn-nouvel-utilisateur" onClick={() => navigate('/backoffice/utilisateurs/new')}>
      ...
      <Card>
        {isLoading && users.length === 0 ? (
          ...
          <EmptyState title="Aucun utilisateur" message="Aucun compte backoffice trouvé." />
        ) : (
      ...
      <div className="text-sm text-subtext">{meta.total} utilisateur{meta.total > 1 ? 's' : ''}</div>
    </div>
  );
}
