...
import { getB2BPalierLabel } from '../../utils/organisationBilling';
...
export default function AbonnementB2BCallback() {
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        const data = await organisationApi.getAbonnementB2B();
      ...
      <Card className="space-y-5">
        <div>
          <h1 className="text-2xl font-semibold text-primary">
            {succes ? 'Paiement B2B reçu' : 'Paiement non abouti'}
          </h1>
          <p className="mt-2 text-sm text-subtext">
            {succes
              ? "Votre paiement a été transmis. Le palier B2B est en cours d'activation."
              : "Le paiement n'a pas pu être traité. Votre palier B2B n'a pas été activé."}
          </p>
        </div>
        ...
        {succes && !checking && abonnement && (
          <div className="space-y-3 rounded-lg border border-success bg-success/5 p-4">
            ...
            <div className="flex items-center justify-between">
              <span className="text-sm text-subtext">Palier</span>
              <span className="font-semibold text-text">{getB2BPalierLabel(abonnement.palier)}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm text-subtext">Capacité</span>
              <span className="font-semibold text-text">{abonnement.nb_max} apprenants</span>
            </div>
            {orderId && (
              <div className="flex items-center justify-between">
        ...
        {succes && !checking && !abonnement && (
          <div className="rounded-lg border border-warning bg-warning/5 p-4 text-sm text-warning">
            La confirmation est en cours de traitement. Votre palier B2B sera activé sous quelques minutes.
            Consultez votre espace B2B pour vérifier le statut.
          </div>
        )}
        ...
        {!succes && (
          <div className="rounded-lg border border-danger bg-danger/5 p-4 text-sm text-danger">
            ...
            Vous pouvez réessayer depuis la page de souscription B2B.
          </div>
        )}
        ...
        <div className="flex flex-wrap gap-3">
          {succes ? (
            <Link to="/organisation/b2b">
              <Button variant="primary" data-testid="btn-voir-b2b">
                Voir mon abonnement B2B
              </Button>
            </Link>
          ) : (
            <Link to="/organisation/b2b">
              <Button variant="primary" data-testid="btn-reessayer-b2b">
                Réessayer
              </Button>
            </Link>
          )}
